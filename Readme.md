...
A self-hosted Kubernetes platform running across an on-prem Ubuntu laptop and Oracle Cloud ARM instances. Built to learn production-grade platform engineering — observability, security, persistent storage, log aggregation, remote access, and AI integration.
...
I wanted a place to experiment with Kubernetes beyond tutorials and sample applications. Instead of just deploying workloads, I focused on building the platform around them — monitoring, logging, security scanning, persistent storage, remote access, and operational tooling. The goal is to keep expanding this environment while documenting what I learn along the way.
...
<details>
<summary>Longhorn Dashboard</summary>
...
To verify the alerting pipeline end-to-end, I intentionally scaled a deployment's replicas to zero. This triggered `KubeDeploymentReplicasMismatch` in Prometheus, which Alertmanager picked up and routed to the configured Slack channel. This confirmed the full path — Prometheus detection → Alertmanager routing → Slack delivery — was working correctly.
...
<details>
<summary>Trivy Vulnerability Dashboard in Grafana</summary>
...
longhorn-system   csi-provisioner-85886d6599-wkms4                         1/1     Running   0          4h9m
...
monitoring        prometheus-monitoring-kube-prometheus-prometheus-0       2/2     Running   0          168m
...
- **Tailscale service exposure** — Configuring the Tailscale Operator to create per-service proxies instead of exposing the entire node required understanding the Tailscale Kubernetes integration model.
